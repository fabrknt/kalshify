...
export async function fetchEulerData(): Promise<IndexData> {
    try {
        console.log("Fetching Euler index data...");
...
export async function calculateEulerScore(
    ...
    const indexData = data || (await fetchEulerData());
...
export async function getEulerCompanyData(
    ...
    const indexData = data || (await fetchEulerData());
    const indexScore =
        score || (await calculateEulerScore(indexData));
