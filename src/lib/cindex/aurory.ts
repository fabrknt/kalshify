...
export async function fetchAuroryData(): Promise<IndexData> {
    try {
        console.log("Fetching Aurory index data...");
...
export async function calculateAuroryScore(
    ...
    const indexData = data || (await fetchAuroryData());
...
export async function getAuroryCompanyData(
    ...
    const indexData = data || (await fetchAuroryData());
    const indexScore =
        score || (await calculateAuroryScore(indexData));
