...
export async function fetchJitoData(): Promise<IndexData> {
    try {
        console.log("Fetching Jito index data...");
...
export async function calculateJitoScore(
    ...
    const indexData = data || (await fetchJitoData());
...
export async function getJitoCompanyData(
    ...
    const indexData = data || (await fetchJitoData());
    const indexScore =
        score || (await calculateJitoScore(indexData));
