...
export async function fetchSafeData(): Promise<IndexData> {
    try {
        console.log("Fetching Safe index data...");
...
export async function calculateSafeScore(
    ...
    const indexData = data || (await fetchSafeData());
...
export async function getSafeCompanyData(
    ...
    const indexData = data || (await fetchSafeData());
    const indexScore =
        score || (await calculateSafeScore(indexData));
